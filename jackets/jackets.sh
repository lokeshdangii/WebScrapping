# https://api.tatadigital.com/api/v1/search/composite?limit=21&q=jackets&scope=Fashion&sort=relevance&currentPage=2

for ((i = 1; i <= 805; i++))
do

  url="https://api.tatadigital.com/api/v1/search/composite?limit=21&q=jackets&scope=Fashion&sort=relevance&currentPage=$i"
  response=$(curl  -H 'authority: api.tatadigital.com' \
                   ...
                   --compressed $url | jq '.results.products'  >> jackets_curl.txt)
